...
الهدف: تصنيف آخر رسالة في المحادثة إلى واحدة من 4 حالات:
  ...
  4. "inappropriate"  -> ألفاظ غير لائقة/شتايم (بالمعنى، حتى لو مش موجودة حرفياً
                          في قائمة الكلمات الممنوعة الثابتة - تحايل بالحروف، شتيمة
                          غير شائعة، تلميح أو سخرية مهينة، إلخ)
...
VALID_CATEGORIES = ("clean", "debate", "offtopic", "inappropriate")

SYSTEM_PROMPT = """أنت مساعد مراقبة لجروب تليجرام دراسي. مهمتك تحليل آخر رسالة في محادثة (بالاستعانة بالسياق اللي قبلها) وتصنيفها إلى واحدة من 4 حالات بالظبط:
...
3) "offtopic" — هزار، مزح، نكت، أو نقاش (حتى لو هادئ ومن غير خلاف) عن موضوع
   ...
   ومفيهاش ألفاظ غير لائقة (لو فيها يبقى "inappropriate" مش "offtopic")

4) "inappropriate" — ألفاظ غير لائقة، شتيمة، إهانة، أو سب موجه لعضو تاني،
   حتى لو مكتوبة بطريقة ملتوية أو بحروف متباعدة أو بأسلوب غير مباشر لكن المعنى واضح إنه إهانة أو سب.
   استخدم فهمك للمعنى مش بس الكلمات الحرفية - لو حد بيهين أو بيسب حد تاني فهي "inappropriate" حتى لو الكلمة مش "شتيمة معروفة".

قاعدة مهمة: لو في شك، أو الرسالة قصيرة وغامضة ومحتمل تكون رد فعل عادي، صنّفها "clean". منمنعش الرسالة إلا لو واضح فعلاً إنها مخالفة.

رد بصيغة JSON فقط بدون أي نص إضافي، بالشكل ده بالظبط:
{"category": "clean" أو "debate" أو "offtopic" أو "inappropriate", "topic": "الكورة" أو "الدين" أو "السياسة" أو null, "reason": "سبب مختصر"}"""
...
async def analyze_with_ai(context_messages: list[str]) -> dict:
    ...
    يرجع: {"category": "clean"|"debate"|"offtopic"|"inappropriate", "topic": str|None, "reason": str}
    """
