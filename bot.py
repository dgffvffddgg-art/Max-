...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== لو سليمة بالفلتر التقليدي، نفحصها بالـ AI لاكتشاف جدال كورة/دين/سياسة =====
    ai_result = {"violation": False, "topic": None, "reason": None}
    if not result["violation"] and config.AI_MODERATION_ENABLED:
        ...
        ai_result = await ai_moderation.check_debate_violation(context_msgs)
    ...
    is_violation = result["violation"] or ai_result["violation"]
    ...
    reason = ai_result["reason"] if ai_result["violation"] else result["reason"]
    logger.info(
...
async def cmd_laws(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يعرض قوانين الجروب لأي عضو يطلبها"""
    await update.message.reply_text(config.GROUP_LAWS_TEXT)
